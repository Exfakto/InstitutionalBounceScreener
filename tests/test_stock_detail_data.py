...
class StockDetailDataTest(unittest.TestCase):
    ...
    def test_nee_candidate_detail_populates_existing_analytics(self):
        ...
        self.assertEqual(len(detail["price_history"]), 2)
        self.assertEqual(detail["price_history"][-1]["close"], 73.25)
        self.assertEqual(detail["trade_levels"]["support"], 68.75)
        self.assertAlmostEqual(detail["trade_levels"]["ideal_buy_zone_low"], 68.0)
        self.assertIn("professional_research_summary", detail["metrics"])
        self.assertIn("trade thesis", detail["metrics"]["professional_research_summary"].lower())
    ...
    def test_nee_candidate_detail_window_has_no_blank_analytics_tabs(self):
        ...
        self.assertEqual(window.price_chart.rows[-1]["close"], 73.25)
        self.assertEqual(window.price_chart.ema_values["ema20"], 72.1)
